                        ...
                        return (
                            <>
                                <Box key={blog.id} variant="outlined" position={'relative'} mt={4}>
                                    <ThemeProvider theme={theme}>
                                        <Typography fontWeight={'bolder'} variant="h5" gutterBottom>
                                    ...
                                    <Typography mt={1} fontWeight={'bold'}>
                                        {`${blog.author.first_name && blog.author.last_name ? `${blog.author.first_name} ${blog.author.last_name}` : blog.author.username}`}
                                    </Typography>
                                    <Typography mt={1} fontSize={'smaller'} color={'text.secondary'}>
                                        {`${blog_day_name} ${blog_month_name} ${blog_day} ${blog_year}`}
                                    </Typography>
                                    <Button size="small" href={`/blog/${blog.id}`} sx={{ position: 'absolute', bottom: 2, right: 2 }} >Read more</Button>
                                </Box>
